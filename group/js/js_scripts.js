...
$.ajax({
    ...
    success: function (result) {
        ...
        memberList.sort(function(x, y){
            ...
            var time_x = parseInt(member_x[4].substr(0,4));

            var member_y = y.split(",");
            var name_y = member_y[1].split(" ");
            var time_y = parseInt(member_y[4].substr(0,4));
        ...
        for(var i=1; i < memberList.length; i++){
            ...
            //4:Graduate Time
            var curmember = memberList[i].split(",");
            var index = 0;
            switch (curmember[0].toLocaleLowerCase()){
            ...
            if(curmember[2] != "/"){
                member_href.setAttribute('href', curmember[2]);
                //member_info.id = "link_card";
            }

            var member_img_div = document.createElement("div");
            member_img_div.className = "member_img_div";
            //var cur_name = curmember[1].toLocaleLowerCase().replace(" ","");
            var cur_name = curmember[1];
            ...
            member_name.append(curmember[1]);
            ...
            member_research_direction.append(curmember[3]);
